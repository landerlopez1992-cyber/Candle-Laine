// ISO 3166-1 alpha-2 (English names), sorted by name.
...
/** For `<select>`: value = ISO code, label = English name. */
...
export function isValidCountryCode(code: string): boolean {
  return _codes.has(code.trim().toUpperCase());
}
...
/** Normalize legacy free-text location to a country code when possible. */
export function normalizeLocationToCountryCode(raw: string | null | undefined): string {
  if (!raw) return '';
  const t = raw.trim();
  if (!t) return '';
  if (isValidCountryCode(t)) return t.toUpperCase();
